...
int main(int argc, char *argv[])
{
    ...
    int signalvalues[] = {SIGHUP, SIGINT, SIGQUIT, SIGILL, SIGABRT, SIGFPE, SIGKILL, SIGSEGV, SIGPIPE, SIGALRM, SIGTERM, SIGUSR1, SIGUSR2, SIGCHLD, SIGCONT, SIGSTOP, SIGTSTP, SIGTTIN, SIGTTOU, SIGBUS, SIGPROF, SIGSYS, SIGTRAP, SIGURG, SIGVTALRM, SIGXCPU, SIGXFSZ, SIGEMT, SIGIO, SIGINFO, SIGWINCH};
    int i;
...
void genSignalMessage(int status) {
    printf("@Receiving signal from child.\n");
        ...
        case SIGINFO:
            printf("=======Error Detected=======\n");
            printf("CHILD signal: SIGINFO\n");
            printf("\tFile lock lost (unused)\n");
            printf("Child process execution FAILED\n");
            break;
        case SIGWINCH:
            printf("=======Error Detected=======\n");
